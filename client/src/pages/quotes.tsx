...
export default function Quotes() {
  ...
  // Group quotes by property and task/job
  ...
  // Group job quotes separately by property and job
  const quotesByPropertyAndJob = quotes
    .filter(quote => quote.jobId)
    .reduce((acc, quote) => {
      const propertyId = quote.propertyId;
      ...
      if (!acc[propertyId][jobId]) {
        acc[propertyId][jobId] = [];
      }
      acc[propertyId][jobId].push(quote);
      return acc;
    }, {} as Record<number, Record<number, Quote[]>>);
            ...
            <CardContent className="p-0">
              <div className="space-y-6 p-6">
                {propertyTasks.map((task) => {
                  const taskQuotes = quotesByPropertyAndTask[parseInt(propertyId)]?.[task.id]?.filter(quote => !quote.jobId) || [];
